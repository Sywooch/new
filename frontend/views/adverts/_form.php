...
<div class="type-form">
    ...
    <? /*= $form->field( $model, 'subcat_id' )->dropDownList( $subcategory, [ 'prompt' => 'Выберите подраздел' ] ) */ ?>
    ...
    <?= $form->field( $model, 'subcat_id' )->widget( DepDrop::classname(), [
        ...
        'pluginOptions' => [
            'depends'     => [ 'cat-id' ],
            'placeholder' => 'Select...',
            'url'         => Url::to( [ '/adverts/subcat' ], true )
        ]
    ] ) ?>
	...
	<h4>Контактная информация
      ...
      if ( Yii::$app->user->isGuest ) {
          echo '
			<a id="sec-lk-enter" href="' . Url::to( "/user/login" ) . '" title="Войти под своим именем">
